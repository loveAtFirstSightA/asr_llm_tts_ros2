/*
* 离线命令词识别，modified by xinnie@iflytek.com
*/
#include "user_interface.h"
char *mic_inditify_number = "ifly_e2f78h9dy4bb3_tek";
...
/* demo recognize the audio from iflytek 6mic */
...
#if whether_print_log
	printf("states is :%d", iat.audio_status);
#endif
	if (iat.ep_stat != 3)
	{
		ret = sr_write_audio_data(&iat, waveData, waveLen); //写入200ms
	}
	else
	{
#if whether_print_log
...
int create_asr_engine(UserData *udata)
{
	...
	snprintf(asr_params, MAX_PARAMS_LEN - 1,
		...
		vad_eos = 800, asr_denoise = 1, vad_enable = 1, ",
			 _ASR_RES_PATH,
			 SAMPLE_RATE_16K,
			 _GRM_BUILD_PATH,
			 udata->grammar_id);
...
int run_asr(UserData *udata, char *path)
{
	char asr_params[MAX_PARAMS_LEN] = {NULL};
	const char *rec_rslt = NULL;
	const char *session_id = NULL;
	...
	snprintf(asr_params, MAX_PARAMS_LEN - 1,
		...
		grm_build_path = %s, local_grammar = %s, \
		result_type = xml, result_encoding = UTF-8, \
		asr_denoise = 1, vad_eos = 5000, vad_enable = 1,",
			 _ASR_RES_PATH,
			 SAMPLE_RATE_16K,
			 _GRM_BUILD_PATH,
			 udata->grammar_id);
...
Recognise_Result deal_with(char *path, char *jet_path, char *grammer_path, char *bnf_path, char *lex_na)
{
	strcpy(_ASR_RES_PATH, jet_path);		  //离线语法识别资源路径，重要，与麦克风及appid绑定
	strcpy(_GRM_BUILD_PATH, grammer_path); //构建离线语法识别网络生成数据保存路径
	strcpy(_GRM_FILE, bnf_path);			  //构建离线识别语法网络所用的语法文件，用户自修改文件
	...
	if (strcmp(device_index, mic_inditify_number) != 0)
	{
#if whether_print_log
		printf("[log]麦克风设备错误：%d\n", ret);
#endif
		result.whether_recognised = false;
		strcpy(result.fail_reason, "mic_device_detect_error");
		goto exit;
	}
#if whether_print_log
	printf("[log]麦克风设备检测成功：%d\n", ret);
#endif
	//device_index = "iflytek";
	ret = MSPLogin(NULL, NULL, login_config); //第一个参数为用户名，第二个参数为密码，传NULL即可，第三个参数是登录参数
	if (MSP_SUCCESS != ret)
	{
#if whether_print_log
